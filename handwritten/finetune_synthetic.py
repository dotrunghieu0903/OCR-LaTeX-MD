"""
Fine-tuning script for LaTeX OCR model using LoRA on synthetic dataset.
...
eval_steps = 100  # More frequent evaluation for synthetic data
...
def train_lora(model, train_dataloader, optimizer, scheduler, device, num_epochs, eval_steps, val_dataloader, tokenizer, bleu_metric, local_rank=0):
    """Training loop for LoRA fine-tuning on synthetic data."""
    model.train()
...
# Loading the final evaluation dataset (handwritten data for testing)
...
if master_process:
    print("\nEvaluating on handwritten test set...")
    ...
    test_loss, test_bleu_scores = evaluate_model(
        best_model, eval_dataloader, device, best_tokenizer, bleu_metric, stage='final'
    )
    ...
    import json
    with open(os.path.join(Config.checkpoint_dir, "synthetic_final_results.json"), "w") as f:
